...
// XỬ LÝ LOGIC RIÊNG CHO TRANG SẢN PHẨM (LỌC, PHÂN TRANG)
// ========================================================

const ITEMS_PER_PAGE = 24; 

// --- 1. HÀM TẢI SẢN PHẨM & PHÂN TRANG & TÌM KIẾM ---
async function fetchFilteredProducts() {
    ...
    try {
        ...
        if (searchQuery) {
            ...
            title.innerText = `Kết quả tìm kiếm cho: "${searchQuery}"`;
        } else if (categoryId) {
            query = query.eq('category_id', categoryId);
            title.innerText = "Sản phẩm theo Danh mục";
            ...
            title.innerText = "Sản phẩm cùng phân loại";
        } else if (brandId) {
            query = query.eq('brand_id', brandId);
            title.innerText = "Sản phẩm theo Thương hiệu";
        } else if (familyId) {
            query = query.eq('family_id', familyId);
            title.innerText = "Sản phẩm cùng Dòng";
        } else {
        ...
        if (count === 0 || data.length === 0) {
            container.innerHTML = '<div class="col-span-full text-center text-gray-500 py-10 font-bold">Không tìm thấy sản phẩm nào!</div>';
            return;
        }
        ...
        data.forEach(item => {
            ...
            if (item.discount_price && item.discount_price > 0 && item.discount_price < item.price) {
                ...
                const normalPrice = item.price ? formatCurrency(item.price) : 'Liên hệ';
                priceHtml = `<span class="text-kn-orange font-black text-sm">${normalPrice}</span>`;
            }
        ...
        if (totalPages > 1) {
            ...
            if (currentPage > 1) {
                paginationContainer.innerHTML += `<a href="${buildUrl(currentPage - 1)}" class="px-3 py-2 bg-white border border-gray-300 text-gray-500 hover:bg-gray-50 rounded-md text-sm font-medium transition">&laquo;</a>`;
            }
            ...
            if (currentPage < totalPages) {
                paginationContainer.innerHTML += `<a href="${buildUrl(currentPage + 1)}" class="px-3 py-2 bg-white border border-gray-300 text-gray-500 hover:bg-gray-50 rounded-md text-sm font-medium transition">&raquo;</a>`;
            }
        }
...
// --- 2. TẢI SIDEBAR & GIỮ TRẠNG THÁI CHECKBOX ---
...
// --- 3. BỘ NÃO: XỬ LÝ KHI KHÁCH BẤM CHỌN BỘ LỌC ---
